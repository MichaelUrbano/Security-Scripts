...
@test "Permissions: /etc/shadow has correct permissions" {
    ...
    run stat -c "%a %U %G" /etc/shadow
    [ "$status" -eq 0 ]

    # Ubuntu/Debian: 640 root shadow
    # RHEL: 0 root root
    if [ "$output" = "640 root shadow" ] || [ "$output" = "0 root root" ] || [ "$output" = "400 root root" ] || [ "$output" = "600 root root" ] || [ "$output" = "000 root root" ]; then
        true
    else
        # Fail
